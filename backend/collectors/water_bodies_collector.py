"""
Water Bodies Data Collector.
Retrieves water bodies and hydrological features from OpenStreetMap.
"""
...
class WaterBodiesCollector(DataCollector):
    """Collects water bodies data from OpenStreetMap."""
    ...
    def collect(self, polygon: Polygon) -> RawDataset:
        """
        Collect water bodies data for the given polygon.

        Args:
            polygon: Validated polygon defining area of interest

        Returns:
            RawDataset with water features

        Raises:
            DataCollectorError: If collection fails
        """
        try:
            logger.info(f"Collecting water bodies for polygon area {polygon.area_sqkm:.2f} sqkm")
            ...
            logger.info(f"Retrieved {len(features)} water features from OSM")
            ...
            raise DataCollectorError(f"Water bodies collection failed: {str(e)}")
    ...
    def _build_overpass_query(self, bbox: str) -> str:
        """
        Build Overpass QL query for water bodies.
    ...
    def _parse_osm_response(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Error parsing water bodies response: {str(e)}")
    ...
    def _way_to_feature(self, way: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Failed to convert water way {way.get('id', '?')}: {str(e)}")
            return None
    ...
    def _relation_to_feature(self, relation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Failed to convert water relation {relation.get('id', '?')}: {str(e)}")
            return None
